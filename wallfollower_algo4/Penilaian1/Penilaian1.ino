...
void setup() {
  ...
  pinMode(powerFinish, INPUT_PULLUP);  // Assuming the switch connects the pin to GND when pressed
}

void loop() {
  int switchState = digitalRead(powerSwitch);
  int switchFinish = digitalRead(powerFinish);

  if ((switchState == HIGH && switchFinish == HIGH) || (switchState == LOW && switchFinish == LOW)) {
    digitalWrite(in1L, LOW);
    digitalWrite(in2L, HIGH);
    digitalWrite(in1R, HIGH);
    digitalWrite(in2R, LOW);
    analogWrite(enL, 0);
    analogWrite(enR, 0);
  } else if (switchState == LOW && switchFinish == HIGH) {
  ...
  else if (switchState == HIGH && switchFinish == LOW) {
    digitalWrite(trigLeft, LOW);
    delayMicroseconds(2);
    digitalWrite(trigLeft, HIGH);
    delayMicroseconds(10);
